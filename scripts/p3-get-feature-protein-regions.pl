...
=head1 Return Significant Protein Regions for Features

    p3-get-feature-protein-regions.pl [options]

This script takes as input a list of PATRIC feature IDs and outputs the protein regions of interest associated with each.
...
while (! eof $ih) {
    ...
    my $resultList = P3Utils::get_data_batch($p3, protein_region => $filterList, $selectList, $couplets, 'patric_id');
    # Print them.
