=head1 Return All Sequence Features Variants in BV-BRC

    p3-all-sfvts [options]

This script returns the IDs of all the sequence feature variants in the BV-BRC database. It supports standard filtering
...
if ($opt->fields) {
    ...
    my ($selectList, $newHeaders) = P3Utils::select_clause($p3, sfvt => $opt, 1);
    # Compute the filter.
    my $filterList = P3Utils::form_filter($opt);
    # We must always have a filter, so add a dummy here.
    push @$filterList, ['ne', 'sfvt_id', 0];
    # Write the headers.
    P3Utils::print_cols($newHeaders);
    # Process the query.
    my $results = P3Utils::get_data($p3, sfvt => $filterList, $selectList);
    # Print the results.
