...
while (! eof $ih) {
    ...
    my $resultList;
    if ($opt->selective) {
        $resultList = P3Utils::get_data_batch($p3, alt_feature => $filterList, $selectList, $couplets, 'genome_id');
    } else {
        $resultList = P3Utils::get_data($p3, alt_feature => $filterList, $selectList, genome_id => $couplets);
    }
